...
public interface BlogRepository extends JpaRepository<Blog, Long> {
    ...
    @Query(
        value = "select distinct blog from Blog blog left join fetch blog.author",
        countQuery = "select count(distinct blog) from Blog blog"
    )
    Page<Blog> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct blog from Blog blog left join fetch blog.author")
    List<Blog> findAllWithToOneRelationships();

    @Query("select blog from Blog blog left join fetch blog.author where blog.id =:id")
